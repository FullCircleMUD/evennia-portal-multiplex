...
"""Logging shim for evennia-portal-multiplex.

Every line the library emits goes to its own ``portalmultiplex.log``,
co-located with Evennia's other logs under ``settings.LOG_DIR``, so debugging
the multiplex means reading one file rather than picking it out of the main
server log.
...
the server logs use — so a multiplex line and a `server.log` line can be read
...
bootstrapped — ``portal_multiplex_log`` is a silent no-op. The import is lazy
and an ImportError is swallowed; the library deliberately does not fall back
to stderr or a local file.
...
_LOG_FILENAME = "portalmultiplex.log"
_VALID_LEVELS = ("INFO", "WARN", "ERROR")


def portal_multiplex_log(message: str, level: str = "INFO", trace: bool = False) -> None:
    """Emit one line to ``portalmultiplex.log``.
